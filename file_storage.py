    ...
    def open(self, file_name_prefix=''):
        ...
        with open(self.file_name + '.dict', 'r') as temp_dict:
            for line in temp_dict:
                ...
                if len(parts) < 3:
                    continue
                self._dict[parts[0]] = (int(parts[1]), int(parts[2]))
    ...
    def write(self, key, value):
        ...
        if key not in self._dict:
            ...
            self.dict_file.write(
                key + '\t' + str(offset_begin) + '\t' + str(offset_end - offset_begin) + '\n'
            )
            self._dict[key] = (offset_begin, offset_end - offset_begin)
